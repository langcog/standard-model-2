...
```{r fig-demographics, fig.width=11, fig.height=5.5}
#| label: fig-demographics
#| fig-cap: ""
# Per-kid BLUPs (xi = intercept, zeta = log-age slope) from the four D_log
# fits joined with Wordbank demographics. For each (language, outcome) we
# regress the z-scored random effect on sex + birth order + caregiver ed.
# Coefficients are in SDs of the outcome and directly comparable across
# panels and languages.
...
coefs <- bind_rows(lapply(seq_len(nrow(combos)), function(i) {
  ...
  mutate(
    term_label = recode(term,
      "sexMale"                  = "Male\n(vs Female)",
      "birth_order_2Later"       = "Later\n(vs First)",
      "caregiver_ed_3College"    = "College\n(vs ≤SomeCol.)",
      "caregiver_ed_3Graduate+"  = "Graduate+\n(vs ≤SomeCol.)"),
    predictor = factor(case_when(
      grepl("sex",          term) ~ "Sex",
      grepl("birth_order",  term) ~ "Birth order",
      grepl("caregiver_ed", term) ~ "Caregiver education")),
    outcome = factor(outcome, levels = c("xi", "zeta"),
                     labels = c("Efficiency (ξ)",
                                "Change in efficiency (ζ)")),
    lang_label = factor(LANG_LABELS[lang_slug],
                        levels = LANG_LABELS[ladder$langs]))

ggplot(coefs, aes(term_label, estimate, color = lang_label)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high),
                  position = position_dodge(width = 0.6), size = 0.35) +
  facet_grid(outcome ~ predictor, scales = "free_x", switch = "x") +
  scale_color_manual(values = LANG_PAL[ladder$langs], name = "language") +
  labs(x = NULL, y = "regression coefficient (z-scored outcome)") +
  theme_minimal(base_size = 9) +
