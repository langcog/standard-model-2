...
varying in their overall learning efficiency and their acceleration
rate. This model provides a good characterization of longitudinal growth
across languages. Applied to data from 24 languages, it suggests that
the female advantage in early vocabulary results from differences in
overall learning efficiency, while socioeconomic differences may
compound through effects on acceleration rate. Using dense observations
of children's early language environment and word recognition speed, the
model also can be used to estimate that XYZ--XYZ\% of variation between
children is due to differences in language input, while XYZ--XYZ\% is
due to measurable differences in word recognition. In contrast to
...
Figure~\ref{fig-demographics} shows the coefficients from this analysis,
along with a cross-language meta-analytic estimate. The data show the
female advantage across all included datasets, but this effect manifests
itself primarily in efficiency (\(\beta = 0.56\), \(p = < .001\)),
rather than acceleration (\(\beta = 0.27\), \(p = < .001\)). In
contrast, maternal education effects were present in both measures, but
stronger in acceleration ( efficiency: \(\beta = 0.1\), \(p = .008\);
acceleration: \(\beta = 0.34\), \(p = < .001\)). One interpretation of
