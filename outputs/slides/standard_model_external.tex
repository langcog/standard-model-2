...
% =====================================================================
\begin{frame}{Prior accumulator models: what each predicts}{Mechanisms by which acceleration ($\kappa_{\text{pop}} > 1$) and variability could arise}

\small
\begin{description}\setlength\itemsep{0.4em}
\item[McMurray (2007).]
  Each word $w$ has an acquisition threshold $\tau_w$, drawn from a
  population distribution $g(\tau)$. Vocabulary at age $t$ is the
  CDF $G(t)$. \textbf{Acceleration} requires $g'(\tau) > 0$
  (i.e., the threshold density is rising as $\tau$ increases over
  the relevant range) --- otherwise growth is sub-linear in age.
  No per-child variation.

\item[Mitchell \& McMurray (2009).]
  Adds a leverage feedback: $a(t) = t + c \cdot L(t)$. They prove
  this can shift growth earlier or later but \emph{cannot create}
  acceleration not already present in $g(\tau)$.

\item[Hidaka (2013).]
  Closed-form AoA distributions for cumulative
  ($N$-accumulator, AoA $\sim$ Gamma) and rate-change
  (1-accumulator at rate $\delta \cdot t^{D}$, AoA $\sim$ Weibull)
  processes. \textbf{$D$ is the rate-change exponent}: ability
  grows as $t^{D+1}$, so $D > 0$ produces acceleration.
  Population-averaged; no per-child structure.

\item[Mollica \& Piantadosi (2017).]
  Per-word Gamma waiting time: AoA $\sim \Gamma(k_w, \lambda_w)$.
  Acceleration is embedded in per-word rate $\lambda_w$ (same
  population for all kids).

\item[Kachergis et al.\ (2021).]
  1-PL Rasch IRT with per-child $\theta_i$ and linear age covariate.
  \textbf{First model with per-child intercept variation} ($\theta_i$
  free), but no per-child acceleration variation.
\end{description}

\end{frame}

% =====================================================================
\begin{frame}{Where prior models sit in the parameter space}{None has both per-child efficiency $\sigma_\alpha$ and per-child slope $\sigma_\kappa$ free}
...
\midrule
\textbf{Best-fitting model (this work)} & \textbf{free}     & \textbf{free}     & \textbf{free} \\
\bottomrule
\end{tabular}
...
\emph{and} $\sigma_\kappa$ free. The two signatures we measure ---
acceleration and variability --- are what these models cannot
\emph{jointly} produce.
