...
% =====================================================================
\begin{frame}{The structural null: pure accumulator}{Where prior models sit, and what they all lack}

\begin{columns}[T, onlytextwidth]
\column{0.55\textwidth}
\textbf{Pure accumulator.}
Each child accumulates word-evidence at per-child input rate $r_i$;
word $j$ is acquired when threshold $\psi_j$ is crossed.

\vspace{0.4em}
\textbf{Per-child scaling exponent} $\boldsymbol{\kappa_i}$:
ability scales as (cumulative tokens)$^{\kappa_i}$.

\begin{itemize}\setlength\itemsep{0.2em}
\item $\kappa_i = 1$ for every child: pure unit accumulator.
\item Between-child variance is determined entirely by $\sigma_r$.
\item No per-child slope variation around the population mean.
\end{itemize}

\vspace{0.4em}
\textbf{Two estimands diagnose deviation from the null:}

\vspace{0.3em}
\centering
$\boldsymbol{\sigma_\alpha}$ \;\;-- \emph{between-child efficiency variation}\\
$\boldsymbol{\kappa_i}$ posterior \;\;-- \emph{is $\kappa$ above 1, and does it vary?}
...
bucket fills past threshold $\psi_j$ (Kachergis et al.\ 2021).}
\end{columns}

\end{frame}

% =====================================================================
\begin{frame}{Where prior accumulator models sit}{None has both per-child efficiency and per-child slope variation}
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{per-child} $\boldsymbol{\sigma_\alpha}$ & \textbf{per-child} $\boldsymbol{\sigma_\kappa}$ & \textbf{$\boldsymbol{\kappa_{\text{pop}} > 1}$} \\
\midrule
McMurray (2007) -- pure accumulator       & --        & --        & only via $g'(\tau)$ \\
Mitchell \& McMurray (2009) -- + leverage & --        & --        & shifts but cannot create \\
Hidaka (2013) -- closed-form AoA          & --        & --        & via rate-change exponent $D$ \\
...
\begin{columns}[T, onlytextwidth]
\column{0.55\textwidth}
\textbf{Common gap.} \emph{No prior accumulator model has both
$\sigma_\alpha$ \emph{and} $\sigma_\kappa$ free.} Most have neither.
The two signatures we measure --- variability and acceleration in growth
rate --- are precisely what these models cannot produce.

\column{0.45\textwidth}
\textbf{Implication for the next slides.} The 4-panel comparison shows
each model's predictions for the canonical CDI percentile fan. The
nested family makes clear what each addition buys.
\end{columns}
...
$\eta_{ijt} = \underbrace{\xi_i}_{\text{child}}
            + \underbrace{\kappa_i}_{\text{scaling exponent}}\!\!\log\!\tfrac{t}{a_0}
...
\end{itemize}
\textbf{Scaling exponent} $\kappa_i \sim \mathcal{N}(\kappa_{\text{pop}}, \sigma_\kappa^2)$:
ability scales as (tokens)$^{\kappa_i}$. McMurray's pure accumulator
sits at $\kappa_i = 1$ for every child.
...
\item $\kappa_{\text{pop}}, \sigma_\kappa$: population scaling and between-child variation in scaling.
\end{itemize}
