...
Population scaling exponent $1+\delta \gg 1$. \\[0.6em]
...
% =====================================================================
\begin{frame}{Cross-language replication}{Variability and growth-with-age in $\sim$30 Wordbank languages}

\centering
\includegraphics[width=0.94\linewidth]{precursor_crosslang_panel.png}

\vspace{0.3em}
\small
\textbf{Variability is universal, and grows with age in every sampled language.}
1-PL Rasch IRT on Wordbank cross-sectional WS data, per-admin
$\theta$ extracted, SD computed per integer age bin.

\end{frame}

% =====================================================================
\begin{frame}{The estimand: a pure accumulator as the structural null}

\begin{columns}[T, onlytextwidth]
\column{0.55\textwidth}
\textbf{Pure accumulator (McMurray 2007).}
Each child accumulates word-evidence at a per-child input rate $r_i$.
Word $j$ is learned when threshold $\psi_j$ is crossed.

\vspace{0.5em}
\textbf{Predictions.}
\begin{itemize}\setlength\itemsep{0.3em}
\item Ability $\propto \log(\text{tokens})$, slope = 1.
\item Between-child variance proportional to $\sigma_r^2$.
\item No per-child acceleration deviation from population.
\end{itemize}

\vspace{0.5em}
\textbf{We measure deviations from this null.}
Two estimands carry the structural argument:

\vspace{0.3em}
\centering
$\boldsymbol{\delta}$ \;\;-- \emph{population age-acceleration}\\
$\boldsymbol{\sigma_\zeta}$ \;\;-- \emph{between-child growth-rate variance}
...
bucket fills past threshold $\psi_j$. Originally Kachergis et al.\ 2021.}
\end{columns}

\end{frame}

% =====================================================================
\begin{frame}{Linear predictor}{Item-response theory + accumulator dynamics}

\centering
$P(\text{produces}_{ijt} = 1) = \sigma\!\big( \eta_{ijt} \big)$

\vspace{0.4em}

\Large
$\eta_{ijt} = \underbrace{\xi_i}_{\text{child}}
            + \underbrace{(1+\delta+\zeta_i)}_{\text{age dynamics}}
              \log\!\tfrac{t-s}{a_0}
            \;-\; \underbrace{\psi_j}_{\text{word}}$
...
\begin{columns}[T, onlytextwidth]
\column{0.5\textwidth}
\textbf{Child intercept} $\xi_i = \log r_i + \log\alpha_i$.
\begin{itemize}\setlength\itemsep{0.2em}
\item $\log r_i$: input rate, externally pinned via Sperry / HR / WF.
\item $\log\alpha_i$: efficiency multiplier (free).
\end{itemize}
\textbf{Acceleration} $1+\delta+\zeta_i$: scaling exponent on
log-tokens. Population mean $\delta$, per-child deviation $\zeta_i$.

\column{0.5\textwidth}
\textbf{Word threshold} $\psi_j$: log-tokens needed to acquire word
$j$, hierarchical by lexical class.

\vspace{0.5em}
\textbf{Headline derived quantities:}
\begin{itemize}\setlength\itemsep{0.2em}
\item $\pi_\alpha = \sigma_\alpha^2 / (\sigma_\alpha^2 + \sigma_r^2)$: efficiency-share of intercept variance.
\item $\sigma_\zeta$: between-child growth-rate variance.
\end{itemize}
...
\vspace{0.4em}
\small
\textbf{Left:} ability $\theta_{it}-\mu_r$ vs.\ $\log_{age}$, slope = $1+\delta+\zeta_i$, mean $\approx 10.4$.\quad
...
$\boldsymbol{\delta}$ & \textbf{9.35 \,[9.21, 9.51]} & \textbf{Population age-acceleration: ability scales as $T^{1+\delta}$.} \\
$\boldsymbol{\sigma_\zeta}$ & \textbf{3.47 \,[3.14, 3.84]} & \textbf{Between-child variation in growth rate.} \\
...
\centering\textbf{\color{accent}Growth-rate variance}\\[0.2em]
\small
...
\textbf{Off-pivot:} growth-rate variance $\sigma_\zeta^2 \cdot \log_{age}^2$ dominates by 30 mo.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Sample} & \textbf{admins/kid} & $\boldsymbol{\delta}$ & $\boldsymbol{\sigma_\zeta}$ & \textbf{mean of $1+\delta+\zeta_i$} \\
\midrule
English  & median 3 & 9.4 & 3.5 & $\sim$10.4 \\
Norwegian & median 8 & 11.5 & 3.7 & $\sim$12.5 \\
\bottomrule
\end{tabular}
...
\begin{columns}[T, onlytextwidth]
\column{0.55\textwidth}
\textbf{Population scaling exponent $\sim10\!-\!12$ in both languages.}

\vspace{0.5em}
What differs across languages is the \emph{decomposition}. With dense
longitudinal data (Norwegian: 8 admins/kid), per-child slopes
$\zeta_i$ are well-identified, and population $\delta$ alone --- without
$\zeta_i$ --- actually \emph{hurts} the fit ($-235$ ELPD).

\vspace{0.5em}
With sparse data (English: 3 admins/kid), $\zeta_i$ is hard to pin
down, and $\delta$ pools the action.
...
between-child variation in $1+\delta+\zeta_i$ spans roughly $5\!-\!15$.}
...
\item $\pi_\alpha = 0.90$ \,$[0.86, 0.94]$ --- replicates within the
  Peekbank subset.
\item $\rho(\zeta_i, \mathrm{rtslope}_i) = -0.02$ \,$[-0.33, +0.27]$
...
between-instance variance in scaling rate ($\sigma_\zeta = 3.5$ vs.\ ~0
