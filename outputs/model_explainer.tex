...
\paragraph{Single-mode posterior, but slow mixing.}
The centered parameterization \(s_i \sim \mathrm{Normal}_+(0, \sigma_s)\)
gives a single-mode posterior (all 4 chains agree on per-chain medians
to within \(\sim5\%\) on every variance component). A non-centered
\(s_i = \sigma_s \cdot s_i^{\text{raw}}\) was tried first and produced
a bimodal posterior (one mode at \(\sigma_s \approx 36\) with
\(\sigma_\alpha \approx 0.9\), one at \(\sigma_s \approx 6.4\) with
\(\sigma_\alpha \approx 2.9\)); the multiplicative factorization
allowed trading kid-onset variance for kid-efficiency variance in
geometrically distant configurations. Centered fixes this.

However, the joint posterior on \((\sigma_\alpha, \sigma_\zeta,
\sigma_s)\) is poorly conditioned: even at 4000 iterations and
\texttt{adapt\_delta} = 0.99, \(R\)-hat sits at 1.5--1.9 on the
slopes\_si variants. The chains explore the same single mode but
slowly within it; point estimates are stable across chains, but the
credible intervals are not yet publication-tight. With independent
\(s_i\):
\(\sigma_\alpha = 1.86\,[1.64, 2.10],\)
\(\sigma_\zeta = 3.10\,[2.85, 3.41],\)
\(\sigma_s = 4.14\,[3.28, 5.01],\)
\(\delta = 8.06\,[7.73, 8.36].\)

\paragraph{Correlation structure (sidebar finding).}
The \texttt{si\_corr} variant frees three pairwise correlations on the
kid-level random effects. The bivariate (\(\xi, \zeta\)) correlation
flips to near zero (\(\rho_{\xi\zeta} = -0.04\)) while the two new
correlations come in large and opposite in sign:
\(\rho_{\xi s} = +0.64\) and \(\rho_{\zeta s} = -0.63\).
Substantively: kids with higher efficiency \(\xi\) tend to have a
larger latent delay (start later, ``catch-up learners''); kids with
steeper acceleration \(\zeta\) tend to have smaller latent delay
(``early bloomers''). This is consistent with separately-documented
Wordbank patterns relating onset timing to later vocabulary
composition (Frank et al., Wordbank book ch.~13, ch.~15) but
mirroring those patterns from the modeling side is a marginal
contribution: the LOO improvement over independent-\(s_i\) is
\(\Delta = 62\) (SE = 8) -- statistically significant but practically
small given the added complexity of three free correlations. For the
headline reporting we lean on the independent-\(s_i\) variant.
