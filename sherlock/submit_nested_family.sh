#!/usr/bin/env bash
# Submit the new pieces of the M0..M6 nested family on English. Most
# fits are already on disk under their established names; this script
# only needs to submit the new ones.
#
# 7-stage spine (factors "+ time" and "+ frequency" as separate steps):
#   M0  long_m0                     no time, no freq, no slopes, no 2PL
#   M1  long_m1_time_only           + time only (delta + freq pinned at 0)
#   M2  long_m1                     + frequency (delta pinned at 0)
#   M3  long_baseline               + free delta
#   M4  long_slopes                 + per-child zeta
#   M5  long_class_beta_slopes      + class-specific beta_c
#   M6  long_m5                     + 2PL (lambda_j)
#
# Plus no_freq_slopes as an off-spine M4-level robustness test.

set -euo pipefail
cd "$HOME/standard_model_2"

echo "Linking input bundles into \$SCRATCH..."
./sherlock/sync_inputs.sh

submit_long() {
  local variant="$1" dataset="$2"
  local jid
  jid=$(sbatch --parsable sherlock/long_fit.slurm "$variant" "$dataset")
  echo "  $variant/$dataset -> job $jid"
}

echo
echo "Submitting nested-family fits (English only)..."
submit_long long_m0              english
submit_long long_m1_time_only    english
submit_long long_m1              english
submit_long long_m5              english
submit_long long_no_freq_slopes  english

echo
echo "Already on disk (skipping):"
echo "  M3 = long_baseline"
echo "  M4 = long_slopes"
echo "  M5 = long_class_beta_slopes"
echo
echo "Queue status:"
squeue -u "$USER" -o "%.10i %.22j %.2t %.10M %.20S"
