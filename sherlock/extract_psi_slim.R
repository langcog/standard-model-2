## Extract per-item psi medians and means, one item at a time, to keep
## memory bounded under 16 GB. The naive approach of pulling all psi
...
extract_one <- function(tag) {
  ...
  if (!"psi" %in% all_vars) { cat("No psi in this fit; skipping.\n"); return(NULL) }

  # cmdstanr $summary handles psi[] as an array; one call returns 1 row
  # per index. This is the lightest path -- doesn't materialize draws.
  s <- fit$summary(variables = "psi", "mean", "median")
  s$jj <- as.integer(sub("psi\\[(\\d+)\\]", "\\1", s$variable))
  s <- s[order(s$jj), c("jj", "mean", "median")]
  names(s) <- c("jj", "psi_mean", "psi_median")
