...
\paragraph{Why we pin \(s\) by default.}
Empirically, when \(s\) is freed jointly with \(\delta\), the two
parameters trade off along a near-perfect ridge: posterior
...
\((1+\delta)/(t-s) = (1+\delta')/(t-s')\), forcing \(\delta=\delta'\)
and \(s=s'\).  Practically the curve-shape difference between two
points along the ridge is small.  For the English age range
(16--30~mo), the ridge endpoints \((s, \delta) = (0.5, 9.4)\) and
\((3, 8)\) produce \((1+\delta)\log\!((t-s)/a_0)\) curves that differ
by only \(\sim 0.1\) logits in shape (after subtracting a constant
that \(\xi_i\) absorbs). With Bernoulli observations and finite \(N\),
that shape difference is at the noise floor. So \(s\) is
\emph{near-non-identifiable in practice}, and the reported \(s\)
posterior in any \texttt{free\_s} variant is only a coordinate choice
along the ridge — it does not deliver an interpretable estimate of
``onset of accumulation.''  We pin \(s\) at \(\sim 0\) (lean baseline)
and treat \texttt{free\_s} as a robustness check, not a first-class
estimate.

\paragraph{Reference age \(a_0\) is dataset-specific.}
\(a_0\) is the pivot at which \(\log_{age} = 0\) and so \(\theta_{it}
= \xi_i\) (per-child intercept).  Setting a single global \(a_0=20\)
across datasets makes \(\xi_i\) language-relative in a misleading way:
when admin ages skew far from \(a_0\), the per-kid likelihood ridge in
\((\xi_i, \zeta_i)\) is tilted by an amount proportional to
\(\log(\bar t_i / a_0)\), pulling the marginal \(r(\xi, \zeta)\)
across kids in one direction or the other.  We observed this as a
spurious sign flip in \(\rho_{\xi\zeta}\) between English (median
admin age 19; \(r=+0.39\)) and Norwegian (median admin age 26;
\(r=-0.32\)) on the same model.  Re-evaluating each kid's \(\xi\) at
their personal mid-observation age makes the flip vanish (Norwegian
\(r\) becomes \(+0.08\)).  We therefore set \(a_0\) per dataset to the
median admin age, in each \texttt{prepare\_*.R} script.  This is a
reparameterization, not a model change: the likelihood, \(\delta\),
and \(\zeta_i\) are invariant; only the \emph{interpretation} of
\(\xi_i\) changes (from ``ability at 20~mo'' to ``ability at the
dataset's median admin age'').
...
\section{Lean baseline + opt-in variants}

After running through several rounds of fits with all the bells and
whistles attached by default, we settled on a deliberately \emph{lean}
baseline and a small set of variants that opt in to extra parameters
explicitly.

\paragraph{Lean baseline (the default).} Includes:
\begin{itemize}[leftmargin=1.3em]
\item Per-child latent \(\xi_i\) (intercept; carries the
  \(\sigma_\alpha\) / \(\pi_\alpha\) decomposition via the
  externally-pinned \(\sigma_r\)).
\item Per-word \(\psi_j\) with class-hierarchical prior.
\item Frequency \(\log p_j\) entering the linear predictor with unit
  coefficient (kept separate so RQ1 can be answered as
  \(r(\psi_j, \log p_j)\)).
\item Population acceleration exponent \(\delta\), free.
\item Class means \(\mu_c\) and SDs \(\tau_c\).
\end{itemize}

\paragraph{Pinned at zero by default.} Both for parsimony and because
they are weakly identified in cross-sectional data:
\begin{itemize}[leftmargin=1.3em]
\item Start time \(s = 0\) (pinned via near-zero prior SD).
\item 2PL discrimination \(\lambda_j \equiv 1\).
\item Per-child slope deviations \(\zeta_i \equiv 0\).
\end{itemize}

\paragraph{Variants opt in.}

\begin{center}
\small
\begin{tabular}{@{}lll@{}}
\toprule
Variant & Adds & Question / use \\
\midrule
\texttt{slopes} & \(\zeta_i,\, \sigma_\zeta\) & Do children vary in growth rate? Default for longitudinal. \\
\texttt{2pl} & \(\lambda_j,\, \sigma_\lambda\) & Do words vary in acquisition sharpness? \\
\texttt{2pl\_slopes} & both & Comparison; defends dropping 2PL when it adds little. \\
\texttt{free\_s} & free \(s\) & RQ2 robustness check. \\
\texttt{fix\_delta} & \(\delta = 0\) & Ablation: no acceleration. \\
\bottomrule
\end{tabular}
\end{center}

The running log of fits and what each revealed is in
\texttt{notes/experiments.md}.

\paragraph{Why we changed defaults.} The earlier defaults included
free \(s\), 2PL, and slopes. Three findings drove the simplification:
(a) 2PL never measurably improved population-level fit even though
\(\sigma_\lambda \approx 0.27\) is non-zero (item discrimination
exists but the population averages it out); (b) start time \(s\) was
poorly identified, hitting prior bounds in cross-sectional fits and
trading off strongly with \(\delta\); (c) per-child slopes
\(\sigma_\zeta\) were unidentifiable from cross-sectional data
(needed longitudinal). The lean default is the smallest model that
addresses every RQ; the variants exist to test specific claims.

\section{Datasets and observation channels}
\label{sec:channels}

The variant grammar (\S\,Lean baseline) toggles \emph{model
