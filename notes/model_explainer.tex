...
children \(\times\) 200 items per dataset. English: median 3 admins,
12 mo span; Norwegian: median 8 admins, 14 mo span (Norwegian has the
much denser Kristoffersen contributor).
...
\paragraph{Cross-sectional Wordbank (single CDI per child).}
The subset used for the cross-sectional and \(\sigma_r\) sensitivity
fits (500 children, 200 items) was sampled by uniform random selection
within age bins (5 monthly groups) and within lexical class (5 classes,
40 items each). Stratification on item difficulty was not used in that
sample; results are robust to this because all 5 classes span the
full frequency range when sampled uniformly within each.
