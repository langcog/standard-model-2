## FINAL developmental-ladder analysis: 5 seeds x 11 rungs (0.5M..24M words),
...
##      (0.434/ParamScale on log10(words)), comparable to training-axis 0.74
##      and kids' ~10
##   3. between-seed sigma of the per-seed kappa -> the LM sigma_kappa analog
##
## Input:  outputs/feng_eval/ladder_bestval.csv
## Output: outputs/figs/longitudinal/ladder_development_final.png
##         outputs/feng_eval/ladder_kappa_summary.csv

suppressPackageStartupMessages({library(dplyr); library(readr); library(ggplot2); library(tidyr); library(patchwork)})

d <- read_csv("outputs/feng_eval/ladder_bestval.csv", show_col_types = FALSE) |>
...
cat(sprintf("CV = %.1f%%   |   kids: kappa ~10, sigma_kappa ~3.5 (CV ~35%%)\n",
...
g <- (p1 | p2) + plot_annotation(
  title = "LM development across 5 individuals (seeds): near-identical, decelerating, shallow",
  subtitle = sprintf(paste0("Left: competence vs distinct input, 11 budgets to convergence; between-seed slope SD %.3f (CV %.0f%%). ",
                            "Right: per-word dev-axis kappa, median %.2f — vs children kappa ~10, sigma_kappa ~3.5 (CV ~35%%)."),
...
ggsave("outputs/figs/longitudinal/ladder_development_final.png", g, width = 10.5, height = 4.6, dpi = 150)
cat("\nwrote outputs/figs/longitudinal/ladder_development_final.png\n")
