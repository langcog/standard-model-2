## EN + NO M_best side-by-side quantile-fan plot for slides.
## Cross-sectional empirical (one admin per kid) with lambda=50 smoothing.
##
## Output: outputs/figs/longitudinal/m_best_quantile_EN_NO.png
...
build_panel <- function(language_label,
  ...
  ## Empirical: cross-sectional, one admin per kid (from bundle$df)
  ...
  ggplot(emp_scatter, aes(age, vocab)) +
    ...
    labs(x = "Age (months)",
         ...
         title = title,
         subtitle = sprintf("%d kids (1 admin/kid x-sec)", nrow(emp_df))) +
...
p_en <- build_panel(
  ...
  cache_path      = file.path(PATHS$fits_dir, "demo_xsec_empirical_long.rds"),
  language_filter = "English (American)",
  title           = "English (I=500, J=671)"
)
p_no <- build_panel(
  language_label  = "NO",
  bundle_path     = file.path(PATHS$fits_dir, "long_subset_data_nor.rds"),
  tag             = "long_no_freq_slopes_norwegian",
  cache_path      = file.path(PATHS$fits_dir, "demo_xsec_empirical_long_NO.rds"),
  language_filter = "Norwegian",
  title           = "Norwegian (I=500, J=674)"
)

combined <- (p_en | p_no) +
  plot_layout(guides = "collect") +
  plot_annotation(
    title = "M_best at full data scale: α + ζ + δ (no s, no s_i)",
    subtitle = "Empirical: grey dots (x-sec scatter) + dashed lines (gcrq, λ=50). Predicted: colour fan."
  ) &
  theme(legend.position = "bottom")

out_png <- file.path(OUT_DIR, "m_best_quantile_EN_NO.png")
