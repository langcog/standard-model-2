// Comp/std-channel IO model WITH signed per-child trajectory phase s_i.
// Extends log_irt_long_io_comp.stan by adding a signed,
// sum-to-zero-centered s_i and a sigma_s hyperparameter, so the time
// term becomes log(t - s - s_i[ch]) in both the production and
// comprehension channels. No (sigma_total, p_zeta) reparam (see
// io_accel_si_signed for rationale).
//
...
model {
  ...
  if (N_comp > 0) {
    vector[N_comp] eta_c;
    {
      vector[N_comp] ae_c;
      for (n in 1:N_comp) {
        int ch_n = admin_to_child[aa_comp[n]];
        ae_c[n] = fmax(admin_age[aa_comp[n]] - s - s_i[ch_n], 0.01);
      }
