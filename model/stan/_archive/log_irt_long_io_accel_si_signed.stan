// Input-uptake + acceleration model WITH signed per-child trajectory
// phase s_i. Extends log_irt_long_io_accel.stan by adding a signed,
// sum-to-zero-centered s_i and a sigma_s hyperparameter, so the time
// term is log(t - s - s_i[ch]) instead of log(t - s). Same break of
// the (s_i, delta) population-mean ridge as in log_irt_long_si_signed.
//
// We do NOT use the (sigma_total, p_zeta) reparameterization from the
// longitudinal base. At BabyView N=20 (and similarly small IO samples)
// the data don't drive the (sigma_zeta, sigma_s) sampling ridge into
// a problem; direct sampling of sigma_s with a half-normal prior is
// adequate.
//
...
data {
  ...
  // ---- Per-child trajectory phase s_i prior ----
  real<lower=0> sigma_s_prior_sd;     // pin tiny (e.g. 0.001) to disable
}
...
parameters {
  ...
  // Per-child trajectory phase (signed; sum-to-zero centered in TP).
  vector[I] s_i_raw;
...
transformed parameters {
  ...
  // Per-child trajectory phase, sum-to-zero centered.
...
model {
  ...
  // Per-child trajectory phase (signed normal, sum-to-zero centered in TP).
  ...
  vector[N] eta;
  {
    ...
    for (n in 1:N) {
      ...
      ae[n] = fmax(admin_age[aa[n]] - s - s_i[ch_n], 0.01);
    }
