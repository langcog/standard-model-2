// Longitudinal log-linear IRT + LWL ladder -- the "BILINGUAL lean" (bi-lean) model.
//
// = the io-proc lean model + a SUMSCORE count branch (Spanish + sumscore extension).
// Bilingual item difficulties need NO model change: English and Spanish items just get
// distinct lexical-class groups (cc / C set by the bundle), so each language's delta_j forms
// its own hierarchy while the population geometry (mu_r, kappa_pop, gamma_in, beta_xi, sigmas)
// is SHARED across languages. The only addition: a sumscore admin (k of n produced on a form)
// enters as a Normal-approx Poisson-binomial over that form's items --
//   k ~ Normal( sum_j p_j, sqrt(sum_j p_j(1-p_j)) ),  p_j = inv_logit(theta_i(age) - delta_j).
// (Lean model description follows.)
//
...
data {
  ...
  // ---- Sumscore count likelihood (Poisson-binomial, Normal-approx over a form's items) ---- //
  int<lower=0> n_sum;                                  // sumscore admins (ELENA-WS, HABLA, ...)
  array[n_sum] int<lower=1, upper=I> sum_child;
  vector[n_sum] sum_log_age;                           // log(age / a0)
  array[n_sum] int<lower=0> sum_k;                     // observed words produced
  array[n_sum] int<lower=1> sum_form;                  // index into the form item-sets
  int<lower=0> n_forms;
  int<lower=0> n_form_items;                           // flattened (form -> item) membership
  array[n_form_items] int<lower=1, upper=J> form_item;
  array[n_forms] int<lower=1> form_start;              // 1-based start of each form's block
  array[n_forms] int<lower=0> form_len;                // n items on each form
}
