// Longitudinal log-linear IRT + LWL processing ladder -- "io-proc LEAN, RASCH-simplified".
//
// Phase-0 simplification of log_irt_long_proc_dp_joint_lean.stan. Three subtractions, each
// removing machinery that was already pinned/dead or is not used in this paper:
//   (1) RIP lambda (item discrimination). Was pinned to ~1 (sigma_lambda=0.001) -> pure
//       Rasch: eta = admin_base + item_offset. Makes "sumscore is sufficient for theta" exact.
//   (2) DELETE s (developmental start time). Was pinned to ~0 (s_prior_sd=0.001); the (s,delta)
//       ridge is gone. theta uses log(age / a0).
//   (3) RIP log_p / beta_c (the structural word-frequency offset). Not used in this paper, so
//       item_offset = -delta_j.
// Everything else (RT level-only channel, input measurement model, regression ladder, the
// lexical-class difficulty hierarchy mu_c/tau_c) is unchanged. delta prior widened in driver.
