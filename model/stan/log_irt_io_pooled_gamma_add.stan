// Pooled hierarchical IO model + ADDITIVE input-on-slope term (gamma).
//
// Identical to log_irt_io_pooled.stan EXCEPT the per-child slope gains a
// term gamma * log_r_dev[i]: a child's input deviation shifts not just the
// ability intercept (xi) but also the accumulation RATE. This is the
// model-based version of the reduced-form check (zeta_i ~ log_r_dev_i,
// which gave BabyView +1.19, SEEDLingS +0.37).
//
//   slope_i = 1 + delta + study_delta + gamma * log_r_dev_i + zeta_i
//
// gamma is in slope (kappa) units, directly comparable to the reduced-form
// coefficient. zeta_i is now the slope deviation NOT explained by input.
//
// Identification: log_r_dev enters the intercept as a level shift and the
// slope as an age-interaction (different signatures in age), so gamma is
// separable from the intercept's input term given the age range. Watch the
// gamma x sigma_r posterior correlation.
...
data {
  ...
  // input-on-slope prior (gamma in slope/kappa units)
...
parameters {
  ...
  real gamma;                     // input-on-slope coefficient (shared)
...
model {
  ...
  {
    // per-child slope (ADDITIVE input term) and per-item offset
    vector[I] slope_child;
    for (i in 1:I)
      slope_child[i] = 1 + delta + study_delta[study_of_child[i]]
                       + gamma * log_r_dev[i] + zeta[i];
