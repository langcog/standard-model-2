// Longitudinal log-linear IRT accumulator with SIGNED s_i + REPARAMETERIZED
// variance channels. Combines two mixing fixes:
//
//   1. Signed-normal s_i (deviations around population mean) instead of
//      half-normal (delays from population). Sum-to-zero centered so
//      mean(s_i) = 0 exactly across kids, regardless of sigma_s -- this
//      breaks the (sigma_s, delta) population-mean compensation ridge
...
// delay. A kid with s_i < 0 is "ahead" of the population reference (their
// log_age at calendar age t is larger), s_i > 0 is "behind." Analogous
// to how xi_i and zeta_i are deviations from population means rather than
// one-sided modifications.
//
// The asymmetry of log() means "late" kids (s_i > 0) get a bigger
// downward pull on log_age than "early" kids (s_i < 0) get an upward
// pull -- e.g. at calendar age 16 with population s ~ 0, a kid 5 mo
// late has log((16-0-5)/19) = -0.547, a kid 5 mo early has
// log((16-0+5)/19) = +0.100. This nonlinearity is what makes s_i good
// at anchoring the q10 floor at young ages.
//
...
parameters {
  ...
  // parameters has empirical mean exactly 0 across kids.
  vector[I] s_i_raw;
...
transformed parameters {
  ...
  // Sum-to-zero center on xi, zeta, and s_i (same pattern across all
