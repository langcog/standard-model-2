// LWL processing-channel model WITH signed per-child trajectory phase
// s_i. Extends log_irt_long_proc.stan by adding a signed,
// sum-to-zero-centered s_i and a sigma_s hyperparameter, so the CDI
// time term becomes log(t - s - s_i[ch]). LWL channel is unchanged
// (it uses lwl_log_age which is supplied directly, not derived from s).
// No (sigma_total, p_zeta) reparam (see io_accel_si_signed for
// rationale: small N, ridge not a sampling problem).
//
